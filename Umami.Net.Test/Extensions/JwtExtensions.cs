...
//Based on website_id, hostname, ip, userAgent
    ...
    public static string DecodeJwt(string token, string secret = Secret)
    {
        ...
        if (jwtToken == null) throw new Exception("Token is not a valid JWT.");

        // Extract payload claims
        var payload = new StringBuilder();
        foreach (var claim in jwtToken.Claims) payload.AppendLine($"{claim.Type}: {claim.Value}");
