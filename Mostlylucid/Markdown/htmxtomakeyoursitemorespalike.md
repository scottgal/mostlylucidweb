...
## Blog Links (the simpler way)
So the method below CAN be made to work however it's super tricky and a bit awkward.
HTMX really likes to work using the attribute based approach. Using `htmx.ajax` is kind of the 'if all else fails' method and means you need to mess with borwser history. 
Unfortunately browser history using `pushState` `replaceState` and `popstate` is really a bit of a pain in the neck (but can be super powerful for edge cases).
...
In the rare occasion that you DON'T want to use HTMX for particular links you can just add `hx-boost="false"` [to the link](https://htmx.org/attributes/hx-boost/).

## Blog Links (NOTE: I abandoned this approach but keep it here for reference)
